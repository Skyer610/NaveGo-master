function [nav_e] = ins_gnss_sop(imu, gnss, sop, att_mode)
...
n_sop = size(sop.lat,2);
n = 15+2*n_sop; % number of states
r = 3+n_sop;  % number of sensors
...
[kf.F, kf.G] = F_update_sop(upd, DCMbn, imu, 3);
...
for i = 2:LI
    ...
    gdx_s =  find (sop.t >= (imu.t(i) - sop.eps) & sop.t < (imu.t(i) + sop.eps));
    
    if ( ~isempty(gdx_s) && gdx_s > 1)
        
        gdx_g = find (gnss.t >= (sop.t(gdx) - gnss.eps) & gnss.t < (sop.t(gdx) + gnss.eps));
        ...
        [kf.F, kf.G] = F_update_sop(upd, DCMbn, imu, n_sop);
        ...
        %% Matrix H update
                    ...
                    Hs = zeros(n_sop,2*n_sop);
                        for i_H = 1:n_sop
                            ...
                            Hs(i_H,i_H:i_H+1) = [1 0];
                        end
                    Hsop = [Hv Hs];
                    Hgnss = [ O33 I33 O33 O33 O33 O2n;
                O33 O33 Tpr O33 O33 O2n; ];
            kf.H = [Hgnss;
                    Hsop];
