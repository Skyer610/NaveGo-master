function [nav_e] = ins_gnss_sop3(imu, gnss, sop)
...
kf.H = [  O33 I33 O33 O33 O33 O36;
    ...
    O33 O33 Tpr O33 O33 O36; ];
kf.R = diag([sop.std  gnss.stdv gnss.stdm]).^2;
kf.z = [sop.std gnss.stdv, gnss.stdm ]';
...
%         gdx_g = find (gnss.t >= (sop.t(gdx_s) - gnss.eps) & gnss.t < (sop.t(gdx_s) + gnss.eps));
        ...
        zp = Tpr * ([lat_e(i); lon_e(i); h_e(i);] - [gnss.lat(gdx_g); gnss.lon(gdx_g); gnss.h(gdx_g);]) ...
            + (DCMbn * gnss.larm);
        
        % Velocity innovations with lever arm correction
        zv = (vel_e(i,:) - gnss.vel(gdx_g,:) - ((omega_ie_n + omega_en_n) * (DCMbn * gnss.larm ))' ...
        ...
        dtg = gnss.t(gdx_g) - gnss.t(gdx_g-1);
        ...
        %% Matrix H update
                    ...
                    Hs = zeros(3,6);
                        for i_H = 1:3
                            l_sop = [sop.lat(gdx_g,i_H)-kf.xi(4) sop.lon(gdx_g,i_H)-kf.xi(5) sop.h(gdx_g,i_H)-kf.xi(6)];
                            l_sop = l_sop / norm(l_sop);
                            Hv(i_H,:) = [O13 l_sop O19];
                            Hs(i_H,i_H*2-1:i_H*2) = [1 0];
                        end
                    Hsop = [Hv Hs];
                    Hgnss = [ O33 I33 O33 O33 O33 O36;
                              O33 O33 Tpr O33 O33 O36; ];
            kf.H = [Hgnss;
                    Hsop];
...
%         ob(gdx_g) = rank(obsv(kf.F, kf.H));
        ...
        xi(gdx_g,:) = kf.xi';
        xp(gdx_g,:) = kf.xp';
        b(gdx_g,:) = [sop.std gb_dyn', ab_dyn'];
        A(gdx_g,:)  = reshape(kf.A,  1, n^2);
        Pi(gdx_g,:) = reshape(kf.Pi, 1, n^2);
        Pp(gdx_g,:) = reshape(kf.Pp, 1, n^2);
        
            v(gdx_g,:)  = kf.v';
            z(gdx_g,:)  = kf.z';
            K(gdx_g,:)  = reshape(kf.K, 1, n*r);
            S(gdx_g,:)  = reshape(kf.S, 1, r^2);
        end
    end
end
