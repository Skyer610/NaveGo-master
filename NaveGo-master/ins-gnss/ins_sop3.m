function [nav_e] = ins_sop3(imu, gnss, sop)
...
xi = zeros(LS, n);      % Evolution of Kalman filter a priori states
xp = zeros(LS, n);      % Evolution of Kalman filter a posteriori states
z = zeros(LS, r);       % INS/GNSS measurements
v = zeros(LS, r);       % Kalman filter innovations

A  = zeros(LS, n^2);    % Transition-state matrices
Pi = zeros(LS, n^2);    % A priori covariance matrices
Pp = zeros(LS, n^2);    % A posteriori covariance matrices
K  = zeros(LS, n*r);    % Kalman gain matrices
S  = zeros(LS, r^2);    % Innovation matrices
ob = zeros(LS, 1);      % Number of observable states at each GNSS data arriving
...
%         zp = Tpr * ([lat_e(i); lon_e(i); h_e(i);] - [gnss.lat(gdx_s); gnss.lon(gdx_s); gnss.h(gdx_s);]) ...
%             + (DCMbn * gnss.larm);
...
%         ob(gdx_g) = rank(obsv(kf.F, kf.H));
        ...
        xp(gdx_s,:) = kf.xp';
        b(gdx_s,:) = [sop.std];
        A(gdx_s,:)  = reshape(kf.A,  1, n^2);
        Pi(gdx_s,:) = reshape(kf.Pi, 1, n^2);
        Pp(gdx_s,:) = reshape(kf.Pp, 1, n^2);
        
            v(gdx_s,:)  = kf.v';
            z(gdx_s,:)  = kf.z';
            K(gdx_s,:)  = reshape(kf.K, 1, n*r);
            S(gdx_s,:)  = reshape(kf.S, 1, r^2);
