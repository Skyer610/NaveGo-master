function [nav_e] = ins_sop3_odo(imu, gnss, sop, odo)
...
xi = zeros(LO, n);      % Evolution of Kalman filter a priori states
xp = zeros(LO, n);      % Evolution of Kalman filter a posteriori states
z = zeros(LO, r);       % INS/GNSS measurements
v = zeros(LO, r);       % Kalman filter innovations

A  = zeros(LO, n^2);    % Transition-state matrices
Pi = zeros(LO, n^2);    % A priori covariance matrices
Pp = zeros(LO, n^2);    % A posteriori covariance matrices
K  = zeros(LO, n*r);    % Kalman gain matrices
S  = zeros(LO, r^2);    % Innovation matrices
ob = zeros(LO, 1);      % Number of observable states at each GNSS data arriving
...
% Prior estimates
x_imu = [ zeros(1,9), imu.gb_dyn, imu.ab_dyn ]' ;
x_clk = zeros(1, 6)';
kf.xi = [ x_imu'  x_clk']';  % Error vector state 
...
[kf.F, kf.G] = F_update_sop3(upd, DCMbn, imu);
...
kf.H = [  O33 I33 O33 O33 O33 O36;
    O33 I33 O33 O33 O33 O36 ];
kf.R = diag([sop.std  odo.stdv]).^2;
kf.z = [sop.std odo.stdv]';

% Propagate prior estimates to get xp(1) and Pp(1)
kf = kf_update_sop3( kf );
...
for i = 2:LI
    ...
    [qua, DCMbn, euler] = att_update(wb_corrected, DCMbn, qua, ...
        omega_ie_n, omega_en_n, dti, 'quaternion');
    roll_e(i) = euler(1);
    pitch_e(i)= euler(2);
    yaw_e(i)  = euler(3);
    ...
    % Check if there is a new LTE measurement to process at current INS time
    gdx_s =  floor(i/8);
    
    if ( ~isempty(gdx_s) && gdx_s > 1)
        
%         gdx_g = find (gnss.t >= (sop.t(gdx_s) - gnss.eps) & gnss.t < (sop.t(gdx_s) + gnss.eps));
        gdx_o = floor(gdx_s/25)+1;
        if ( ~isempty(gdx_o))
        %                 gdx   % DEBUG
...
%         zp = Tpr * ([lat_e(i); lon_e(i); h_e(i);] - [gnss.lat(gdx_o); gnss.lon(gdx_o); gnss.h(gdx_o);]) ...
%             + (DCMbn * gnss.larm);
        
        % Velocity innovations with lever arm correction
        zv = (vel_e(i,:) - odo.vel3(gdx_o,:) - ((omega_ie_n + omega_en_n) * (DCMbn * odo.larm ))' ...
            + (DCMbn * skewm(wb_corrected) * odo.larm )' )';
        ...
        kf = kf_prediction(kf, dtg);

        %% Matrix H update
                    Hv = zeros(3,15);
                    Hs = zeros(3,6);
                        for i_H = 1:3
                            l_sop = [sop.lat(gdx_o,i_H)-kf.xi(4) sop.lon(gdx_o,i_H)-kf.xi(5) sop.h(gdx_o,i_H)-kf.xi(6)];
                            l_sop = l_sop / norm(l_sop);
                            Hv(i_H,:) = [O13 l_sop O19];
                            Hs(i_H,i_H*2-1:i_H*2) = [1 0];
                        end
                    Hsop = [Hv Hs];
                    Hodo = [ O33 I33 O33 O33 O33 O36;];
            kf.H = [Hodo;
                    Hsop];
...
%         ob(gdx_g) = rank(obsv(kf.F, kf.H));
        ...
        xi(gdx_o,:) = kf.xi';
        xp(gdx_o,:) = kf.xp';
        b(gdx_o,:) = [sop.std gb_dyn'];
        A(gdx_o,:)  = reshape(kf.A,  1, n^2);
        Pi(gdx_o,:) = reshape(kf.Pi, 1, n^2);
        Pp(gdx_o,:) = reshape(kf.Pp, 1, n^2);
        
            v(gdx_o,:)  = kf.v';
            z(gdx_o,:)  = kf.z';
            K(gdx_o,:)  = reshape(kf.K, 1, n*r);
            S(gdx_o,:)  = reshape(kf.S, 1, r^2);
        end
    end
end
